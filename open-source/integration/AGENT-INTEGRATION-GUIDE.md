...
|------------|-------------|------|
| **Discovery & catalog (read)** | Category tree, **discover outline/candidates**, lists, detail, keyword search, health, public points leaderboard | **Valid API key** (`Authorization: Bearer` or `x-api-key`; paths in OpenAPI) |
